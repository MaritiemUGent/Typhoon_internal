...
for s=1:noofwings
	for t=1:(noofloops(s))
      ...
      %ALEC - insert line here for trim angle foil (TODO)
...
for s=1:noofwings
   for t=1:noofloops(s) %setuploop
      [C V N2 P]=geometry19(geo.fnx(s,t),geo.ny(s,t),geo.nx(s,t),...
         ...
         geo.curved(s,t),geo.radius(s,t),geo.foiltheta(s,t));
...
infx=infdist*cos(state.theta)*cos(state.psi);
infy=-infdist*sin(state.psi);
infz=infdist*sin(state.theta)*cos(state.psi);
...
%these next formulas incorporate SWEEP and TWIST
